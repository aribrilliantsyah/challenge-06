...
const prefix = '/api/v1/'
const controller = 'user-game-biodata'
...
describe('User Game Biodata API Test', () => {
  beforeAll(async () => {
    let account = {
  ...
  test(`POST ${path} - Success Create user game biodata`, async() => {
  ...
  test(`DELETE ${path}/${id} - Success Delete user game biodata`, async() => {
  ...
  test(`POST ${path} - Unauthorized Create user game biodata`, async() => {
    const { body, statusCode } = await request(app).post(`${path}`)
    expect(statusCode).toEqual(401)
    expect(body.message).toEqual('Unauthorized')
  })
  ...
  test(`PUT ${path}/2 - Unauthorized Update user game biodata`, async() => {
  ...
  test(`DELETE ${path}/2 - Unauthorized Delete user game biodata`, async() => {
  ...
  // Failed
  test(`POST ${path} - Failed Create user game biodata`, async() => {
    const { body, statusCode } = await request(app).post(`${path}`)
      .send()
      .set({
        Authorization: `Bearer ${token}`
  ...
  test(`POST ${path} - Failed Create user game biodata (2) user_game_id = string`, async() => {
    const { body, statusCode } = await request(app).post(`${path}`)
      .send({
        'user_game_id': 'hh'
      })
  ...
  test(`PUT ${path}/${id} - Failed Update user game biodata`, async() => {
    const { body, statusCode } = await request(app).put(`${path}/${id}`)
      .set({
        Authorization: `Bearer ${token}`
      })
  ...
  // Not Found
  id = 100
  test(`PUT ${path}/${id} - Data not found Update user game biodata`, async() => {
  ...
  test(`DELETE ${path}/${id} - Data not found Delete user game biodata`, async() => {
    const { body, statusCode } = await request(app).delete(`${path}/${id}`)
      .set({
        Authorization: `Bearer ${token}`
      })
  ...
  ug_id = 100
  test(`POST ${path} - User game id not found Create user game biodata`, async() => {
    const { body, statusCode } = await request(app).post(`${path}`)
      .send({
        "user_game_id": ug_id,
        "name": "Ari Ardiansyah",
